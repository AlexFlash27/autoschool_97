...
class SiteController extends Controller
{
    ...
    public function actions()
    {
        return [
            ...
            'captcha' => [
                ...
                'offset' => 1, // расстояние между символами
            ],
        ];
    }
    ...
    public function actionLogin()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->login()) {
            ...
            return $this->redirect(['/user/account?id='.Yii::$app->user->identity->id]);
        } else {
            $model->password = '';
    ...
    public function actionSignup()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->signup()) {
            //Yii::$app->session->setFlash('success', 'Спасибо за регистрацию. Проверьте свою электронную почту на наличие письма с подтверждением.');
            //return $this->goHome();

            Yii::$app->session->setFlash('success', 'Вы зарегистрировали нового пользователя - ' .$model->username. '!');
            return $this->redirect(['/user/index']);
        }
